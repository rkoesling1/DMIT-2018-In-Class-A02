...
namespace eRestaurant.Framework.BLL
{
    ...
    public class SeatingController
    {
        ...
        public List<SeatingSummary> SeatingByDateTime(DateTime date, TimeSpan time)
        {
            ...
            {
                // copy/paste code from LinqPad (adapt)
                #region Adapted from LinqPad
                // Finding out information in the tables in the restaurant at a specific date/time
                // Create a date and time object to use for sample input data
                // var date = context.Bills.Max(b => b.BillDate).Date; // just want the date portion
                // var time = context.Bills.Max(b => b.BillDate).TimeOfDay.Add(new TimeSpan(0, 30, 0));
                ...
                // since the data type of context.Tables is a DbSet<Tables>,
                ...
                // step1.Dump("Results of Setp 1");

                // Step 2 - Union the walk-ins bills and the reservation bills while extracting the
                //			relevant bill info.
                //			.ToList() helps resolve the 
                //			"Types in Union or Concat are constructed imcompatibly"
                ...
                var step4 = from data in step3
                            select new SeatingSummary() // the DTO class to use in my BLL
                            {
                                ...
                                Seating = data.Seating,
                                Taken = data.Taken,
                                // use a ternary expression to conditonally get the bill id (if it exists)
